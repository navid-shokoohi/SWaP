...
class SparseGaussianCRF(BaseEstimator):
    ...
    """
    def __init__(self, learning_rate=1.0, lamL=1, lamT=1, n_iter=10000):
        self.lamL = lamL
        self.lamT = lamT
        ...
        self.beta = 0.5
        self.slack = 0.05
#        self.file = open(global_var.project_path+"/Results/Hourly/gcrf/Lambda/")
    ...
    def fit(self, X, Y):
        ...
        rmse1 = [i for i in range(Y.shape[1])]
        rmse2 = [i for i in range(Y.shape[1])]
        rmse3 = [i for i in range(Y.shape[1])]
        counter = 0;
    ...
    def check_descent(self, newton_lambda, alpha, fixed, vary,it):
        ...
        #if (it%100) == 0:
         #   print("newton Lamda : ",nll_a,"  ::  Lamda : ",nll_b)
        return nll_a <= nll_b
    ...
    def line_search(self, newton_lambda, fixed, vary,it):
        # returns cholesky decomposition of Lambda and the learning rate
        alpha = self.learning_rate
        while True:

            #convert infs and nans in newton_lambda to 0
            where_are_NaNs = np.isnan(newton_lambda)
            newton_lambda[where_are_NaNs] = 0
            newton_lambda[newton_lambda == np.inf] = 0

            pd, L = check_pd(self.Lam + alpha * newton_lambda)
            if pd and self.check_descent(newton_lambda, alpha, fixed, vary,it):
    ...
    def alt_newton_coord_descent(self, X, Y):
        ...
        if self.Lam is None:
            self.Lam = np.eye(q)
            Sigma = np.eye(q) #Sigma is covariance matrix (inverse of lambda)
        else:
            Sigma = inv(self.Lam)
        ...
        for it in pbar(range(self.n_iter)):
            ...
            #Psi is 3rd term in gradient wrt Lambda
            R = np.dot(np.dot(X, self.Theta), Sigma) / np.sqrt(n)
            vary = VariableParams(Sigma=Sigma,
                                  Psi=np.dot(R.T, R))
            ...
            #active_Lam are a set of chosen entries from Lambda (based on gradient wrt Lambda)
            active_Lam = self.active_set_Lam(fixed, vary)
            #newton_lambda is update matrix for Lambda
            newton_lambda = self.lambda_newton_direction(active_Lam, fixed, vary, max_iter=1)

            #find a good learning rate
            learning_rate = self.learning_rate
            LL, learning_rate = self.line_search(newton_lambda, fixed, vary,it)
            #print("LL",LL)
            #update lambda
            ...
            #active_Theta are a set of chosen entries from Theta (based on gradient wrt theta)
            active_Theta = self.active_set_Theta(fixed, vary)

            #update theta
            self.Theta = self.theta_coordinate_descent(active_Theta, fixed, vary, max_iter=1)

            #store lambda and theta on 10th to last iter
            if(it == self.n_iter - 10):
                prevLam = self.Lam
                prevTheta = self.Theta

            #check if lambda and theta converged on last iteration
            if(it == self.n_iter-1):
                lamDif = np.abs(np.subtract(self.Lam, prevLam))
                thetaDif = np.abs(np.subtract(self.Theta, prevTheta))
                #print("lambds",lamDif)
                #print("theta",thetaDif)
                #check if there is a difference greater than 0.001
                if((lamDif > 0.001).sum() > 0):
                    print("Lambda not converged")
                if((thetaDif > 0.001).sum() > 0):
    ...
    def predict(self, X, Y=None):
        ...
        -Θ * Λ^-1 * x
        """
        #ADD ZEROS TO LAMBDA AND THETA FOR MY SPECIFIC TASK
        """self.Lam = np.triu(self.Lam,-1)
        self.Lam = np.tril(self.Lam,1)

        keep_index = 6
        for i in range(0,self.Theta.shape[0]-1):
            self.Theta[i, :keep_index] = 0
            self.Theta[i, keep_index+1:] = 0
            if(keep_index == 6):
                keep_index = 0
            else:
                keep_index += 1
